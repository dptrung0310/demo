...
struct Sprite {
    ...
    int currentFrame = 0;
    int frameDelay = 10; // Đổi giá trị này để điều chỉnh tốc độ chuyển frame
    ...
    void tick() {
        ...
        // Chỉ chuyển frame khi thời gian đạt đến ngưỡng nhất định
        if (frameCount >= frameDelay) {
            currentFrame = (currentFrame + 1) % clips.size();
            frameCount = 0; // Đặt lại biến đếm thời gian
        }
    }
...
struct Graphics {
    ...
    void move(int &x, int &y ,const Sprite& sprite, const Uint8* currentKeyStates)
    {
        y += SPEED_FALL;
            if (currentKeyStates[SDL_SCANCODE_UP] )
        {
            // Nhảy lên với lượng speed nhất định
            y -= speed;
        }
        ...
        if (y + 80 >= 320) {
            y = 240; // Đảm bảo không vượt quá y=320
        }
        const SDL_Rect* clip = sprite.getCurrentClip();
        SDL_Rect renderQuad = {x, y, clip->w, clip->h};
        SDL_RenderCopy(renderer, sprite.texture, clip, &renderQuad);
    }
    /*void gameOver(int &x,int &y , const Sprite& sprite)
    {
